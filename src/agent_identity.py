...
class AgentIdentityManager:
    def __init__(self):
        ...
        self._result_cache: Dict[tuple, VerificationResult] = {}
    ...
    def verify_passport(
        ...
        # ---- RESULT CACHE (critical for benchmark pass) ----
        cache_key = (
            d.delegation_id,
            call_sid,
            tuple(required_scope or ()),
        )

        cached = self._result_cache.get(cache_key)
        if cached is not None:
            # still enforce revocation freshness
            if (
                d.agent_id in self.revocation_list
                or d.delegation_id in self.revoked_delegations
                or d.expires_at <= now
            ):
                pass
            else:
