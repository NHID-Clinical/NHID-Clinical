...
Companion Specification to NHID-Clinical v1.4
Delegated Authority Verification for Healthcare Voice AI

v1.1 — Updated companion reference from NHID-Clinical v1.3 to v1.4. No substantive changes to authorization requirements.
Author: Brianna Baynard
Published: May 2026
License: CC BY 4.0Abstract
NHID-Auth addresses the authorization gap identified in NHID-Clinical v1.4: the ability to
...
NHID-Clinical v1.4 Core requires AI voice agents to disclose their non-human identity before
...
NHID-Auth v1.1 is intentionally minimal to enable immediate adoption while the ecosystem
...
• Integration with FHIR authorization frameworks7. Relationship to NHID-Clinical v1.4 Core
NHID-Auth is a companion specification, not a replacement. The two layers work together:
Layer Addresses Status
NHID-Clinical v1.4 Behavioral disclosure (who is calling, when they disclose, how they escalate) Stable - implementable now (expanded five-test CTS)
NHID-Auth v1.1 Authorization verification (proof of delegation to act on behalf of provider) Baseline - infrastructure maturing
...
• NHID-Clinical v1.4 Core Specification
...
4.0).
Suggested citation:
Baynard, B. (2026). NHID-Auth v1.1 Specification: Authorization Layer for Healthcare Voice
