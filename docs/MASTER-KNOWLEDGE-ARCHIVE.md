...
|---|---|---|
| `UNIT_PUBLISHED` | **1116** | `scripts/validate_ci.py` — published reference, **not** a CI gate |
| `SKIP_EXPECTED` | **0** | `scripts/validate_ci.py` |
| `XFAIL_EXPECTED` | **0** | `scripts/validate_ci.py` |
| Suite, API running | **1116 passed, 0 skipped, 0 xfailed** | `python -m pytest tests/ -q` |
...
| Collected | **1116** across **64** files | `pytest --collect-only -q` |
...
not run by `validate_ci.py`; quote it separately or as a stated combination (**1116 Python + 66
...
# Expected: 1116 passed, 0 skipped, 0 xfailed — with the API running.
...
[![Tests](https://img.shields.io/badge/python%20tests-1116%20passing-brightgreen)](...)
...
UNIT_PUBLISHED = 1116  # scripts/validate_ci.py (published count, not a CI gate)
