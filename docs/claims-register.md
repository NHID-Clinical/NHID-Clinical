...
| DB4 | `specs/…Operational-Blueprint-v1.3.pdf` | No mention of PDX-01 anywhere | **NOT A DEFECT — my earlier flag was wrong.** Checked against the builder (`make_operational_blueprint`): the document has **no control-definitions section at all**. Its only control references are rows in a regulatory mapping table, each citing the controls relevant to that instrument. It states *"5 controls"* and *"all five controls"* and enumerates none. The original flag came from a naive text-presence test — PDX-01 count of zero — which is not evidence of an omission where nothing is enumerated. **The authoritative source does not require a change** |
