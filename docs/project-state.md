...
```
1110 passed, 0 skipped, 0 xfailed   # python -m pytest tests/ -q (API running)
...
1110 collected, 1110 passed. Every published surface stating these numbers must
...
python -m pytest tests/ -q                      # 1110 passed, 0 skipped, 0 xfailed
...
|---|---|---|
| Suite passing | **1110** | `python -m pytest tests/ -q` with the API running |
| Recorded divergences | **0** | both contracts resolved; see `skipped-test-audit.md` §8 |
| Skipped | **0** | same run |
| Collected | **1110** | `--collect-only -q`; equals the passing count |
