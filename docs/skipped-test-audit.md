...
**Published figure: 1015 passing.** Derived by running the suite against a live
API, which is now what CI does in both the `test` and `security_gates` jobs.

> **1015 automated tests pass on every change**, covering the five controls, the
...
- **Never present a collected total as a passing total.** They happen to be equal now (1015) only because nothing is skipped or deferred; if they ever diverge, publish the passing figure.
...
| Is the current public metric wrong? | **It was incomplete, and it is now replaced.** 987 → 1015, and the 18 skips → nothing deferred at all |
