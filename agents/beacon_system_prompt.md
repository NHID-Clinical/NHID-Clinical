...
| 2026-06-25 | documentation only (no agent push) | Added `docs/elevenlabs-beacon-knowledge-base.md`: an operational checklist + drafted document content for a Knowledge Base, scoped tightly to off-script supplementary questions (provider/practice reference data, deeper IDG-02 authorization detail, the alternative-contact-path number/hours referenced vaguely in the EIT-01 fallback line above). Explicitly excludes the disclosure/consent/escalation script itself from the Knowledge Base — that stays governed solely by this file's `prompt` fence, since the CTS runner needs deterministic gate behavior and RAG retrieval is not deterministic. Setup is a dashboard step, same `ELEVENLABS_API_KEY`-blocked pattern as every push above; this row is documentation-only. |
