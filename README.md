...
<p align="center">
  ...
  <img alt="Python Tests" src="https://img.shields.io/badge/python%20tests-1020%20passing-brightgreen?style=flat-square">
...
2. `pip install -r requirements.txt && python -m pytest tests/ -v` → **1020 passing** (0 skipped, 0 xfailed)
...
**Available today**
- Deterministic policy engine with 1020 tests, all passing, across all phases
...
**Engine Validation**:
- ✅ **1020 passing tests** (comprehensive rule coverage across all phases)
...
Comprehensive test suite · same inputs → identical output · **1020 passing**, every collected test executed
...
Expected: **1020 passing** in ~3.0s, with no skips and no xfails. Live demos and full docs on [nhid-clinical.org](https://nhid-clinical.org).
...
| `tests/` | The Python conformance and invariant tests (1020 passing, all phases: foundations, adversarial, synthetic, hardening). |
