...
## 🔐 Badge Schema & Verification Model (Informative)

NHID-Clinical certifications (L1, L2, L3) are issued as **machine-readable compliance credentials**, not static images. This section defines the badge schema and verification model for organizations claiming NHID-Clinical conformance.

This schema is **informative** and sets the foundation for the live public registry (planned v1.4). Organizations receiving certification in v1.3 will use this format for their compliance artifacts.

### Why Machine-Readable Badges Matter

Healthcare payer systems need to verify AI caller credentials programmatically during live call handshakes — not just display a badge on a website. The badge schema provides:

- **Cryptographic integrity** — audit hash detects evidence tampering
- **Automated trust verification** — payers can validate credentials without manual registry lookups
- **Expiration enforcement** — prevents use of expired certifications
- **Revocation support** — allows credential invalidation before expiration
- **Digital signature** — proves issuer authenticity

This is trust infrastructure, not marketing material.
...
{
  "nhid_version": "1.3",
  "badge_level": "L1 | L2 | L3",
  "badge_label": "NHID-LX CERTIFICATION_TITLE",
  ...
  "certification": {
    ...
    "certification_id": "uuid/hash",
    "registry_url": "https://nhid-clinical.org/registry/<id>",
    "revocation_list_url": "https://nhid-clinical.org/revoked"
  },

  "scope": {
    "workflow_type": "B2B-administrative",
    "use_cases": ["claim_status", "eligibility", "prior_auth"]
  },

  "conformance": {
    "cts_version": "1.3",
    "tests_passed": ["IDG-01", "PDX-01", "DBC-01", "EIT-01", "ATR-01"],
    "dfr": "< 0.02",
    "evidence_level": {
      "tier": "tier1 | tier2 | tier3",
      "includes": ["structured_logs", "audio_samples", "third_party_report"]
    }
  },

  "audit": {
    "audit_hash": "sha256-hash-of-evidence",
    "log_format": "FHIR | JSON",
    "sample_size": "integer",
    "last_audit_at": "ISO-8601 timestamp"
  },

  "attestation": {
    "type": "self | reviewed | third-party",
    "auditor": "string | null",
    "signature": "digital-signature-or-hash"
  },

  "trust": {
    "authorization_model": "NPI | token | BAA-linked",
    "deceptive_artifacts": false,
    "pre_data_exchange_enforced": true,
...
The JSON schema drives programmatic verification. Human-readable badges are derived from the schema for display purposes:

**L1 Baseline:**
```
NHID-L1 VERIFIED
Baseline Conformance • CTS Passed
Self-Attested • v1.3
```

**L2 Operational:**
```
NHID-L2 OPERATIONAL CERTIFIED
Production Validated • DFR < 2%
Evidence Reviewed • v1.3
```

**L3 Enterprise:**
```
NHID-L3 ENTERPRISE TRUST CERTIFIED
Independently Audited • Continuous Monitoring
Registry Verified • v1.3
```

### Verification Model

The badge schema enables SSL-style trust verification:

| Verification Element | Purpose | Implementation |
|---------------------|---------|----------------|
| `certification_id` | Primary lookup key | Unique identifier for registry queries |
| `registry_url` | Public verification endpoint | Returns full certification record (v1.4+) |
| `audit_hash` | Integrity check | SHA-256 hash of evidence package detects tampering |
| `signature` | Issuer authenticity | Cryptographic proof badge was issued by NHID-Clinical |
| `expires_at` | Forces revalidation | L3 certifications expire after 12 months |
| `revocation_list_url` | Security incident response | Public list of revoked certifications |

### How It Works (Operational Flow)

1. **Vendor receives certification** → NHID-Clinical issues signed JSON badge + human-readable display version
2. **Vendor implements badge** → JSON embedded in system metadata, display badge shown on website/docs
3. **Payer receives AI call** → AI agent presents `certification_id` during handshake
4. **Payer validates** → queries registry (manual until v1.4, automated after), checks `audit_hash`, verifies `expires_at`
5. **Green lane or reject** → L2/L3 certifications bypass "are you human?" challenge, non-certified or expired badges get standard termination

### Future: JWT Implementation (v1.4 Roadmap)

The badge schema will be packaged as a **signed JWT (JSON Web Token)** for real-time call authentication:

```javascript
// vendor system generates signed token during call initiation
const nhidToken = jwt.sign(
  {
    nhid_version: "1.3",
    badge_level: "L2",
    system: { name: "Retell-Claims-Bot-v2.3" },
    certification_id: "abc-123-xyz",
...
// payer system receives call, validates token signature
const verified = jwt.verify(nhidToken, NHID_PUBLIC_KEY);
if (verified.badge_level === "L2" || verified.badge_level === "L3") {
  // green lane: accept call, skip manual verification
}
```

This enables **zero-latency trust verification** — payers validate AI caller credentials cryptographically during the call handshake, no manual registry lookup required.

### Implementation Note

Systems claiming NHID-Clinical conformance in v1.3 MAY implement this badge schema for internal tracking and external display. Use of the badge schema is not required for L1/L2/L3 certification but is **strongly recommended** for organizations preparing for v1.4 registry integration.

The live public registry (v1.4+) will require all certified systems to use this schema for programmatic verification.
